...
See also the [Legacy Dependencies](#legacy-dependencies) section for what remains in the jQuery-era codebase and the path to full removal.
...
| `CreateOrDestroyDraggableEvent` | `.getType(): string`, `.isCreate(): boolean` | Start/stop the new runtime's context-window drag session |
| `SetDraggableVisibleEvent` | `.getType(): string`, `.isVisible(): boolean` | Show/hide the new runtime's context-window drag chrome |
...
| Phase | Status | Notes |
|-------|--------|-------|
| **Phase 0: Runtime Primitives** | Done | `editor/` runtime bootstrapped in the live editor: shared overlay host, placeholder islands, shared style injection, path registry, DOM parsing for both page and fragment shells, geometry scheduler, bus adapter wiring, path-scoped subtree reconciliation, and mutation-observer safety net are all active on this branch. |
...
```
src/main/resources/assets/js/editor/
├── rendering/
...
- **Context-window inserts** ([`context-window-drag.ts`](../src/main/resources/assets/js/editor/interaction/context-window-drag.ts)): `CreateOrDestroyDraggableEvent` / `SetDraggableVisibleEvent` drive a pointer-based drag session that computes region targets and fires `AddComponentEvent`. The post-drop click suppression is handled by `DragAndDrop.isNewlyDropped()` in the selection handler — no `setNextClickDisabled` guard is armed, because the mousedown originates in the parent document and no synthetic click is produced.
- **Intra-page component reordering** ([`component-drag.ts`](../src/main/resources/assets/js/editor/interaction/component-drag.ts)): Pointer-based drag with 8px threshold, axis-aware insertion index (excluding the source element), nested-layout and fragment-layout containment validation. Fires `MoveComponentEvent`, `ComponentViewDragDroppedEvent`, and all legacy lifecycle events.
...
| 4 | Drag feedback surfaces layer correctly during an active session | Drag state sync, context-window insert targeting, hover suppression, keyboard suppression, and post-drop click suppression stay aligned with legacy drag semantics | Cross-region drag keeps placeholder/highlighter feedback aligned with the active target |
...
The new runtime is a layer on top of the legacy system, not a full replacement. Several legacy dependencies remain in the production bundle and affect the development environment.

### jQuery and jQuery UI

**Status:** Required at runtime. Cannot be removed.

`DragAndDrop.ts` imports `jquery-ui/ui/widgets/sortable` at the module level. The new runtime disables sortables on init, but the module is still loaded because `PageView` instantiates `DragAndDrop` during boot and the module-level imports execute regardless.

jQuery is also a transitive dependency of the external `@enonic/lib-admin-ui` and `@enonic/lib-contentstudio` packages, which provide:
...
The new runtime calls into legacy `ItemView` instances through `bridge.ts` for selection mirroring, drag state, context menu actions, scroll-into-view, and nested-layout validation. These bridge calls remain until the legacy view classes are fully removed from the parent Content Studio frame.
...
3. **Remove legacy view instantiation** from the parent Content Studio frame
...
- **Coexistence**: Legacy view classes and external packages remain; the new runtime bridges into them

Phases 0–4 are complete. Phase 5 (inline rich-text editing) remains in legacy code with no migration planned.
