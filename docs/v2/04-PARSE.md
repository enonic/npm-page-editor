...
function parsePage(root: HTMLElement, descriptors: DescriptorMap): Record<string, ComponentRecord>;
```
...
Same logic scoped to a single subtree. Used after targeted DOM mutations when a full page re-parse isn't needed. Merges into existing registry at the reconcile layer.
...
Note: parse does NOT depend on `state/`. It produces `ComponentRecord` objects but doesn't write to the registry. That's the reconciler's job (step 07).
