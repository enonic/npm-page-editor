...
function PlaceholderIslandMount({
  children,
  style,
}: {
...
function OverlayHostMount({children}: {children: preact.ComponentChildren}): JSX.Element {
  useEffect(() => {
...
function StyleSwatch({label, className}: {label: string; className: string}): JSX.Element {
  return (
...
function TokenGrid(): JSX.Element {
  return (
    <div className='flex flex-col gap-4 p-4'>
      ...
      <p className='text-xs text-subtle'>If colors render correctly, design tokens are working inside shadow DOM.</p>
    </div>
  );
}
    ...
    <OverlayHostMount>
      <div style={{position: 'fixed', top: '80px', right: '24px', pointerEvents: 'auto'}}>
        <div className='w-72 rounded-lg border border-bdr-soft bg-surface-neutral p-4 shadow-lg'>
          ...
          <p className='text-xs text-subtle'>This panel lives in the overlay shadow root with fixed positioning.</p>
