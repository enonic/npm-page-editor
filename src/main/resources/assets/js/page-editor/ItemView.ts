...
import {ItemViewContextMenu, ItemViewContextMenuOrientation} from '@enonic/lib-contentstudio/page-editor/ItemViewContextMenu';
...
export abstract class ItemView
    ...
    private bindMouseListeners() {
        ...
        ResponsiveManager.onAvailableSizeChanged(this, (_item: ResponsiveItem) => {
            if (this.mouseOver) {
                this.highlight();
            }
            if (this.isSelected()) {
        ...
        this.mouseOverViewListener = () => {
            const isRegistered = !!this.getParentItemView() || PageItemType.get().equals(this.getType());
            if (ItemView.debug) {
    ...
    showCursor() {
        const itemView = this.isViewInsideSelectedContainer() ? SelectedHighlighter.get().getSelectedView() : this;
    ...
    handleClick(event: MouseEvent) {
        ...
        const rightClicked = event.which === 3 || event.ctrlKey;
        ...
        if (!this.isSelected() || rightClicked) {
            ...
            const isViewInsideSelectedContainer = this.isViewInsideSelectedContainer();
            const clickPosition = !this.isEmpty() ? {x: event.pageX, y: event.pageY} : null;
            ...
            if (!selectedView || selectedView === this || !isViewInsideSelectedContainer) {
                const menuPosition = rightClicked ? null : ItemViewContextMenuPosition.NONE;
    ...
    protected isEventOverItem(event: MouseEvent): boolean {
        const offset = this.getEl().getDimensions();
        const x = event.pageX;
        const y = event.pageY;
    ...
    static parseItemId(element: HTMLElement): ItemViewId {
        const attribute = element.getAttribute('data-' + ItemViewId.DATA_ATTRIBUTE);
