...
import {type ElementDimensions, type ItemView} from './ItemView';
import type {HighlighterStyle} from '@enonic/lib-contentstudio/page-editor/ItemTypeConfig';
...
export class Highlighter
    ...
    constructor(type?: HighlighterMode) {
        ...
        const svgCls = StyleHelper.getCls('highlighter');
        const html = `<svg xmlns="http://www.w3.org/2000/svg" version="1.1" class="${svgCls}" style="top:-5000px;left:-5000px">
    ...
    highlightItemView(itemView: ItemView): void {
        ...
        const dimensions = itemView.getEl().getDimensions();
        const style = itemView.getType().getConfig().getHighlighterStyle();
    ...
    private resize(dimensions: ElementDimensions, style: HighlighterStyle): void {
        ...
        let strokeW;
        const top = Math.round(dimensions.top);
        const left = Math.round(dimensions.left);
