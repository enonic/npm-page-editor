...
export class PagePlaceholder extends ItemViewPlaceholder {
  ...
  private initListeners() {
    ...
    this.controllerDropdown.onSelectionChanged(
      (selectionChange: SelectionChange<Descriptor>) => {
        if (selectionChange.selected?.length > 0) {
          const pageDescriptor: Descriptor = selectionChange.selected[0];
          new SelectPageDescriptorEvent(
            pageDescriptor.getKey().toString(),
          ).fire();
        }
      },
    );
  ...
  private initElements() {
    ...
    this.controllerDropdown = new PageDescriptorDropdown(
      new ContentId(this.pageView.getLiveEditParams().contentId),
    );
    this.pageDescriptorPlaceholder = new DivEl(
      "page-descriptor-placeholder",
      StyleHelper.getCurrentPrefix(),
    );
  }

  private dataLoadedHandler: (
    event: LoadedDataEvent<Descriptor>,
  ) => Q.Promise<void> = (event: LoadedDataEvent<Descriptor>) => {
    if (event.getData().length > 0) {
      this.controllerDropdown.show();
      const type = new ContentTypeName(
        this.pageView.getLiveEditParams().contentType,
      );
      if (!type.isPageTemplate()) {
        return new GetContentTypeByNameRequest(type)
          .sendAndParse()
          .then((contentType: ContentType) => {
            this.infoBlock.setTextForContent(contentType.getTitle());
          })
          .catch((reason) => {
