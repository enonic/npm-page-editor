...
export class Shader {
    ...
    private resizeToElement(element: Element): void {
        ...
        const documentWidth = Math.max(win.getWidth(), bodyEl.getWidth());
        const documentHeight = Math.max(win.getHeight(), bodyEl.getHeight());

        const dimensions = element.getEl().getDimensions();
        const x1 = Math.max(0, dimensions.left);
        const y1 = Math.max(0, dimensions.top);
        const x2 = Math.min(documentWidth, dimensions.left + dimensions.width);
        const y2 = Math.min(documentHeight, dimensions.top + dimensions.height);
