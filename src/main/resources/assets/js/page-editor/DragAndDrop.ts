...
import type {FragmentComponentView} from './fragment/FragmentComponentView';
...
export class DragAndDrop {
    ...
    handleSortStart(event: JQueryEventObject, ui: JQueryUI.SortableUIParams): void {
        ...
        const regionView: RegionView = this.getRegionView(ui.item.parent());

        let itemType: ItemType;
        const placeholder = DragPlaceholder.get().setRegionView(regionView);
    ...
    handleSortStop(event: JQueryEventObject, ui: JQueryUI.SortableUIParams): void {
        ...
        if (!DragHelper.get().isDropAllowed()) {
            ...
            const componentIndex = $('>.drag-helper, >.' + StyleHelper.getCls('item-view'),
    ...
    handleDragOver(event: Event, ui: JQueryUI.SortableUIParams): void {
        ...
        const regionView: RegionView = this.getRegionView(ui.placeholder.parent());
    ...
    handleRemove(event: Event, ui: JQueryUI.SortableUIParams): void {
        ...
        const fromRegionView = this.getRegionView($(event.target as HTMLElement));
    ...
    handleReceive(event: Event, ui: JQueryUI.SortableUIParams): void {
        ...
        const toRegionView = this.getRegionView($(event.target as HTMLElement));
    ...
    private isDraggingLayoutOverLayout(regionView: RegionView, draggingItemType: ItemType): boolean {
        ...
        if (isFragment || isLayout) {
            if (regionView.hasParentLayoutComponentView() &&
                (isLayout || (this.draggedComponentView as FragmentComponentView)?.containsLayout())) {
                return true;
            }
        }
    ...
    private getComponentView(jq: JQuery): ComponentView {
        const comp = this.pageView.getComponentViewByElement(jq.get(0));
        assertState(!!comp, i18n('live.view.drag.error.compviewisnull'));
        return comp;
    }

    private getRegionView(jq: JQuery): RegionView {
        const region = this.pageView.getRegionViewByElement(jq.get(0));
    ...
    private createSortableItemsSelector(): string {
        ...
        const sortableItemsSelector: string[] = [];
    ...
    private updateScrollSensitivity(selector: Element): void {
        const scrollSensitivity = this.calculateScrollSensitivity();
