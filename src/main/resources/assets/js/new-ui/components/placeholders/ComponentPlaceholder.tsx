  ...
  return (
    <div className='flex min-w-0 flex-1 flex-col gap-2.5'>
      {WIREFRAME_WIDTHS.map(width => (
        <div key={width} className='h-2.5 rounded bg-main/15' style={{width}} />
      ))}
    </div>
  );
}
  ...
  if (error) {
    return (
      <div className='pe-shell rounded-lg border-2 border-error/60 px-6 py-10 text-center'>
        <p className='text-base italic text-error'>{descriptor ?? 'This component could not be rendered.'}</p>
      </div>
    );
  }
  ...
  return (
    <div className='pe-shell rounded-lg border border-bdr-soft bg-surface-primary p-5'>
      <div className='flex items-center gap-4'>
        {icon ? <div className='size-12 shrink-0 text-decorative'>{icon}</div> : null}
        <WireframeLines />
      </div>
    </div>
  );
