...
import {ComponentPath, getComponentInfoAt} from '../../protocol';
import {COMPONENT_SELECTOR, ERROR_ATTR, REGION_SELECTOR} from '../constants';
import {stripFragmentDescendants} from '../dom/fragment';
...
function resolveDescriptor(path: ComponentPath, type: ComponentRecordType): string | undefined {
    ...
    const info = getComponentInfoAt(getPage(), path.toString());
    if (!info) {
        return undefined;
    }

    return info.descriptor || info.fragmentId || info.name || undefined;
}
...
export function childPath(parentPath: ComponentPath, segment: string | number): ComponentPath {
    const base = parentPath.isRoot() ? '' : parentPath.toString();
    return ComponentPath.fromString(`${base}/${segment}`);
}
...
export function parseComponentSubtree(
    ...
    // An embedded fragment is one opaque unit: drop its descendants' tracking
    // attributes so they are not independently selectable and do not shadow the
    // fragment in `closest('[data-portal-component-type]')` hit-testing.
    if (type === 'fragment') {
...
export function parseRootComponent(element: HTMLElement, records: Record<string, ComponentRecord>): ComponentRecord {
    const type = element.dataset.portalComponentType as ComponentRecordType;
...
export function parseRegionSubtree(
    ...
    const path = childPath(parentPath, regionName ?? '');
    const key = path.toString();
